...
export function Globe({
  ...
  const _buildData = () => {
    ...
    for (let i = 0; i < arcs.length; i++) {
      ...
      if (!arc.color) {
        console.warn(`Missing color in arc ${i}`, arc);
        continue;
      }

      console.log('Color value:', arc.color, 'Type:', typeof arc.color);
      const rgb = hexToRgb(arc.color);
      if (!rgb) {
        console.warn(`Invalid color format in arc ${i}:`, arc.color);
        continue;
      }
  ...
  const startAnimation = () => {
    ...
    const validArcs = data.filter(arc => 
      ...
      ...arc,
      color: typeof arc.color === 'function' ? arc.color : () => `rgba(6, 182, 212, 1)`
    }));
    ...
    globeRef.current
      .ringsData([])
      .ringColor((e) => (t) => {
        const colors = ['255, 0, 0', '0, 255, 0', '0, 0, 255']; // Red, Green, Blue
        const colorIndex = e.order % colors.length;
        return `rgba(${colors[colorIndex]}, ${1 - t})`;
      })
...
export function hexToRgb(hex) {
  // Return default color if hex is not a string or is empty
  if (!hex || typeof hex !== 'string') {
    console.warn('Invalid hex color provided:', hex);
    return { r: 0, g: 0, b: 0 };
  }

  // Remove any whitespace and ensure # prefix
  hex = hex.trim();
  if (!hex.startsWith('#')) {
    hex = '#' + hex;
  }

  // Handle both shorthand and full hex formats
